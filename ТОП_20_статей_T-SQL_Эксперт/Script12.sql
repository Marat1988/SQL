...
SELECT ID, ISJSON(StrJSON) AS Status, Header, StrJSON
FROM TestTable
...
SELECT JSON_QUERY(StrJSON, '$.Goods') AS [Все товары в зазазе],
       StrJSON AS [Заказ]
FROM TestTable
WHERE ISJSON(StrJSON)=1
...
--Преобразовыввем их в табличный вид (с WITH)
SELECT *
FROM OPENJSON(@JSON, '$.Goods')
     WITH
     (
